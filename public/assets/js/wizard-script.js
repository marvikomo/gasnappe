		...
		if ($elementToSlide.prev().length) {
			...
			$prev.clearQueue().animate({
				...
				if (activeElementPosition === 1) {
					...
					if ($plugin.find('.aiia-wizard-button-next').hasClass('theme-btn')) {
	...
	function getButtonsTemplate(settings) {
		var $buttons = $("" +
			"<div class='aiia-wizard-buttons-wrapper row'>" +
				"<div class='col-md-12'>" +
					"<button class='btn theme-btn long pull-left aiia-wizard-button-previous'></button>" +
					"<button class='btn theme-btn long pull-right aiia-wizard-button-next' data-loading-text='" + settings.aiiaWizard.localization.buttons.loading + "'></button>" +
				"</div>" +
			"</div>" +
		"").css({
			'display': 'block'
